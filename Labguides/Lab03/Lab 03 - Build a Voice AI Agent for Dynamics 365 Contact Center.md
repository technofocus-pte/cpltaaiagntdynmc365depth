...
This lab guides participants through creating and configuring a voice-enabled AI agent using Microsoft Copilot Studio and Dynamics 365 Contact Center. You will activate the required trial environments, verify the contact center voice setup, create a voice agent, add
knowledge sources, configure conversational topics and live-agent escalation, connect the agent to the voice workstream, and test the solution with a live phone call.

## Lab Objective

Design and deploy a voice-enabled AI agent for Contoso that handles inbound customer calls, provides automated responses to Warranty & Return Policy and Troubleshooting Guide queries, and transfers calls to a live agent when required. You will integrate Microsoft Copilot Studio
with Dynamics 365 Contact Center and verify the agent by placing a live phone call.

## Business Scenario

Contoso wants to modernize customer support by introducing an AI-powered voice agent that can respond to common customer questions without human intervention while providing a seamless escalation path for complex
issues.

## Current challenges

1. High call volume for repetitive queries — Customers frequently contact support with questions about warranty and return policies and troubleshooting guidance, increasing the workload for live agents.

2.  Delayed customer response times — Live agents spend time answering routine policy and troubleshooting questions instead of
    focusing on high-value or complex customer interactions.

3.  To address these challenges, Contoso will deploy a voice-based Copilot agent connected to a Dynamics 365 Contact Center voice
    workstream.
...
6.  Enter the required details, such as **Job title**,
...
**Conclusion:**
Copilot Studio is activated and the correct **ContactCenter Trial** environment is available for agent configuration.
...
1. From the environment selector at the bottom of the page, select **ContactCenterTrial**.
...
**Conclusion**
The Contoso Voice Agent is created and configured with the required authentication setting.
...
**Conclusion**
The Warranty & Return Policy and Troubleshooting Guide documents are added as knowledge sources for the agent.
...
**Conclusion**
The conversation starts and main menu topics are updated, and the unnecessary sample topics are disabled.
...
**Conclusion**
The Warranty and Return Policy topic is configured to use generative AI with the Warranty & Return Policy document as its selected knowledge source.
...
**Conclusion**
The Troubleshooting Guide topic is configured to use generative AI with the Troubleshooting Guide document as its selected knowledge source.
...
**Conclusion**
The escalation topic is configured to transfer calls to a live agent.
...
> **Note: Agent testing may take up to 15 minutes to become available
> after configuration.**

![](./media/image88.png)

Conclusion:\
The Contoso Voice Agent is connected to the voice workstream and is
ready for testing once the connection becomes available.
...
**Conclusion**
The voice agent successfully answers a policy query and routes the call to Copilot Service Workspace when escalation is requested.
